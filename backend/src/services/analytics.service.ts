...
export async function getRevenueChart(query: AnalyticsQuery) {
...
export async function getTransactionStats(query: AnalyticsQuery) {
...
export async function getProviderAnalytics(query: AnalyticsQuery) {
  const { startDate, endDate } = getDateAggregation(query);
...
export async function getSuccessRate(query: AnalyticsQuery) {
  ...
  const grouped: Record<string, { total: number; paid: number; failed: number }> = {};
  data.forEach((item) => {
    const date = item._id.date;
    if (!grouped[date]) grouped[date] = { total: 0, paid: 0, failed: 0 };
    grouped[date].total += item.count;
    if (item._id.status === 'paid') grouped[date].paid += item.count;
    if (item._id.status === 'failed') grouped[date].failed += item.count;
  });
...
function getDateAggregation(query: AnalyticsQuery) {
  const now = new Date();
