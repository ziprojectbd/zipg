...
export const DEFAULT_SETTINGS: Record<string, Record<string, unknown>> = {
  general: {
    ...
    supportEmail: process.env.SUPPORT_EMAIL || 'support@pay.zipremiumservices.com',
  ...
  security: {
    ...
    corsOrigins: (process.env.CORS_ORIGINS || 'https://pay.zipremiumservices.com,https://zipremiumservices.com').split(',').map((o) => o.trim()).filter(Boolean),
  ...
  email: {
    ...
    encryption: 'tls',
    senderName: 'ZI Pay',
    senderEmail: 'noreply@pay.zipremiumservices.com',
  },
  api: {
    apiVersion: 'v1',
    enableApi: true,
